# Design: Batch Collector Deployment
...
`dcf deploy` provisions a batch collector as a scheduled, containerized job. The collector's `deploy:` block declares the schedule; the project's `catalog` setting determines the deployment target (`local` or `gcp`). The design centers on two Docker apps as primary artifacts:

1. **Collector container** — runs `dcf run <collector>` on demand; one per collector; built and destroyed per collector deploy/undeploy
2. **Airflow container** — schedules and triggers collector runs; shared across all deployed collectors; built once on first deploy; not rebuilt when collectors are added or removed
...
DAGs are **not baked into the Airflow image**. They are written as Python files to a directory that Airflow mounts and polls. This means adding or removing a collector does not require rebuilding or restarting Airflow.
...
dcf deploy [<name>]           reads catalog=local from project.yml
    ...
    ├── For each collector:
    │     │
    │     ├── Python: assemble collector build context → /tmp/dcf-collector-<name>/
    │     │
    │     ├── terraform apply  [batch_collector/ {location=local}]
    │     │     ├── local_file.dockerfile → Dockerfile (batch_collector.Dockerfile.tftpl)
...
dcf deploy [<name>]           reads catalog=gcp from project.yml
    ...
    │     ├── Python: assemble collector build context → /tmp/dcf-collector-<name>/
    │     │
    │     ├── terraform apply  [batch_collector/ {location=gcp}]
    │     │     ├── local_file.dockerfile → Dockerfile (batch_collector.Dockerfile.tftpl)
...
dcf undeploy <name>
    │
    ├── terraform destroy  [batch_collector/ {location=local|gcp}]
...
**Dockerfile template:** `dcf/infra/modules/templates/batch_collector.Dockerfile.tftpl`

Contents baked into image: `dcf/` source, `pyproject.toml`, `collectors/<name>.yml`, `connectors/`, minimal `project.yml`
...
| **Type** | service |
| **Owner** | dcf code |
| **Local behavior** | Docker Compose: Airflow scheduler + webserver (port 8080) + PostgreSQL. DAGs read from `~/.dcf/airflow/dags/` (host-mounted volume). `DockerOperator` triggers collector containers via Docker socket. |
| **Deployed behavior** | Cloud Run service (min-instances=1, gen2 execution environment). DAGs read from `gs://<warehouse-bucket>/airflow/dags/` via GCS FUSE mount. `CloudRunExecuteJobOperator` triggers Cloud Run collector jobs. |
...
- Output: triggers collector containers on schedule; webserver UI at port 8080 (local) or Cloud Run HTTPS URL (GCP)
...
**Built once on first deploy.** Rebuilt only if the Airflow version or providers change — not when collectors are added, changed, or removed.
...
One `.py` file per deployed collector. Airflow polls for changes every 30 seconds (`dag_dir_list_interval = 30`).
...
### Terraform Module: `batch_collector/`

| Property | Value |
|----------|-------|
| **Type** | module (parent + two child modules) |
| **Owner** | dcf code |
| **Entrypoint** | `dcf/infra/modules/batch_collector/` |

The parent `batch_collector/main.tf` routes to the appropriate child module via a `location` variable and Terraform `count`:
...
**Child module: `batch_collector/local/`**

| Resource | Description |
|----------|-------------|
| `local_file.dockerfile` | Renders `batch_collector.Dockerfile.tftpl` into build context |
| `null_resource.build` | `docker build -t dcf-local/<name>:latest` |

Variables: `collector_name`, `build_context`, `image_tag`, `content_hash`, `java_enabled` (default: `true`)
Output: `image_tag`

**Child module: `batch_collector/gcp/`**
...
Variables: `collector_name`, `build_context`, `image_uri`, `sa_email`, `content_hash`, `java_enabled` (default: `false`), `project_id`, `region`
Output: `job_name`

**Shared template:** `dcf/infra/modules/templates/batch_collector.Dockerfile.tftpl` — referenced as `${path.module}/../templates/` from child modules (parent is always one level above child in the copied work dir tree).

**State:** `~/.dcf/terraform/collectors/<name>/local/` or `~/.dcf/terraform/collectors/<name>/gcp/`
...
Same parent-routes-to-child pattern as `batch_collector/`. Parent `airflow/main.tf` uses `count` on `module "local"` and `module "gcp"` blocks.
...
| Collector image build | `docker build` via `null_resource` | `gcloud builds submit` via `null_resource` | Same Dockerfile template; different builder |
| Collector image storage | Local Docker daemon | Artifact Registry | |
| Collector execution | Airflow `DockerOperator` → `docker run` | Airflow `CloudRunExecuteJobOperator` → Cloud Run job | Different operator; same collector `CMD` |
...
| Collector storage | Volume-mounted `<project>/warehouse/` | GCS warehouse bucket | |
| Java in collector image | Yes — `java_enabled: true`; Spark used for local Iceberg writes | No — `java_enabled: false`; PyArrow + GCS direct write bypasses Spark | Determined by catalog in image |
| Airflow rebuild needed for collector changes | No | No | Only the DAG file changes; Airflow container is unaffected |
| Terraform state: collectors | `~/.dcf/terraform/collectors/<name>/local/` | `~/.dcf/terraform/collectors/<name>/gcp/` | |
...
```
dcf deploy [<collector-name>]
dcf undeploy <collector-name>
dcf deploy status [<collector-name>]
```

| Command | Behavior |
|---------|----------|
| `dcf deploy` | Deploy all collectors with a `deploy:` block. Apply Airflow module on first deploy (idempotent thereafter). Write DAG file per collector. Exit non-zero if any collector fails; print per-collector summary. |
| `dcf deploy <name>` | Deploy one collector. Same Airflow module apply. Write that collector's DAG file. |
| `dcf undeploy <name>` | `terraform destroy` that collector's module. Delete its DAG file. Airflow stops scheduling within ~30s. |
| `dcf deploy status` | Print deployment state from `project.yml` for all collectors. |

Future: `dcf deploy <name1> <name2> ...` to deploy a named subset.

### Config Schema (`project.yml`)

No new collector YAML fields. New keys in `project.yml`:
...
Collector `deploy:` block is unchanged:
```yaml
deploy:
  schedule: "0 8 * * *"
  paused: false
...
|--------|-----------|--------------|
| `batch_collector/local/` | `local` | `collector_name`, `build_context`, `image_tag`, `content_hash`, `java_enabled` |
| `batch_collector/gcp/` | `gcp` | `collector_name`, `build_context`, `image_uri`, `sa_email`, `content_hash`, `java_enabled`, `project_id`, `region` |
...
Python always invokes the parent module (`batch_collector/` or `airflow/`) and passes `location`. The parent routes via `count` to the appropriate child.

### Inter-Service Protocols

| From | To | Protocol | Payload |
|------|----|----------|---------|
| Airflow scheduler (local) | Collector container | Docker socket — `DockerOperator` | Image tag, env vars, volume mounts |
| Airflow scheduler (GCP) | Cloud Run job | Cloud Run Execute Job API (gRPC) | Job name; collector name is baked into image as `ENV PIPELINE_NAME` |
| Cloud Run collector job | GCS | GCS object write (HTTPS) | Parquet files |
...
|----------|--------|------------------------|-----------|
| Airflow hosting (cloud) | Cloud Run service (min-instances=1, gen2) | Cloud Composer; Compute Engine VM; GKE | Composer is the problem being solved. Compute Engine is always-on + VM management. GKE is heavy. Cloud Run fits the app-centric Terraform pattern and is consistent with collector container deployment. |
...
- **`airflow standalone` scale limit.** Single-process Airflow is not recommended for high-scale production. Acceptable for a personal data lake. Future: migrate to Celery executor + separate scheduler/webserver services if collector count grows.
- **`dcf deploy` subset targeting.** Currently all-or-nothing (no args) or single collector. Future: `dcf deploy <name1> <name2> ...` to deploy a named subset.
...
| 2026-05-14 | Parent module (`batch_collector/`, `airflow/`) with `location` variable routes to child modules via `count`; replaces flat `batch_collector_local/` and `gcp/batch_collector/` structure | Single entry point from Python; location semantics are explicit; GCP child module resources are only initialized when `location=gcp` so local-only users need no GCP credentials. `count=0` on GCP module avoids provider initialization at apply time. | A Terraform pattern that handles conditional provider initialization at init time (not just apply time) changes the tradeoff |
| 2026-05-14 | Replace Cloud Composer with custom Airflow Docker app | Composer is outside Terraform's control (imperative gcloud). Custom Docker app is declarative, version-controlled, and follows the same local/cloud pattern as collector containers | Airflow operational complexity becomes too burdensome |
| 2026-05-14 | DAGs mounted from directory; not baked into Airflow image | Image baking requires Airflow rebuild on every collector change. Mounted directory lets dcf write/delete DAG files and Airflow picks up changes in ~30s with no restart. Keeps undeploy = terraform destroy + delete file. | DAG count or size grows large enough to make GCS FUSE latency a problem |
...
| 2026-05-14 | `java_enabled: true` for local, `false` for GCP | Local catalog uses Spark (requires JVM). GCP catalog uses PyArrow + GCS direct write, bypassing Spark (`runner.py:19`, `iceberg.py:63`). | A GCP collector type is added that requires Java |
...
| 2026-05-14 | `dcf deploy` (no args) deploys all collectors independently; exits non-zero if any fail | Independent deploys allow partial success. All-or-nothing would abort healthy collectors on one failure. | N/A |
