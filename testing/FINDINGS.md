...
| Level | Definition |
|-------|-----------|
| **Blocking** | This type of pipeline cannot be built at all with dcf in its current form |
...
| F-050 | `gcp/batch_pipeline/main.tf` named Cloud Run job `"pvc-job-${var.pipeline_name}"` — stale prefix from before project rename | `dcf/infra/modules/gcp/batch_pipeline/main.tf` — renamed to `"dcf-job-${var.pipeline_name}"` | batch-deployment-gcp |
...
| F-043 | `google_dataflow_flex_template_job` not in GA Terraform provider — `hashicorp/google` does not support this resource type | `dcf/infra/modules/gcp/streaming_pipeline/main.tf` — switched `required_providers` to `hashicorp/google-beta ~> 5.0`; added `provider = google-beta` on the resource | |
| F-042 | `dcf undeploy` would call `terraform destroy` (cancel) not drain on a Dataflow job | `infra/modules/gcp/streaming_pipeline/main.tf` — `on_delete = "drain"` on the `google_dataflow_flex_template_job` resource; Terraform handles the drain automatically | |
| F-041 | No Beam runner code in dcf — no pipeline to read from Pub/Sub, project, and write windowed Parquet to GCS | New `dcf/gcp/beam_runner.py` — `ReadFromPubSub → project_message → FixedWindows → WriteToParquet` Beam pipeline; runs as Dataflow Flex Template entrypoint | |
| F-040 | No `streaming_pipeline` Terraform module — batch module only provisions `google_cloud_run_v2_job` | New `dcf/infra/modules/gcp/streaming_pipeline/` — `google_dataflow_flex_template_job` with `on_delete = "drain"` | |
| F-039 | `dcf deploy` could not load or route streaming pipelines | `dcf/cli.py` — `deploy` and `undeploy` commands route by `pipeline.deployment.type`; `deploy-status` and confirm messages are streaming-aware | |
...
| F-037 | `source.type: pubsub` not recognized — `Source` union only accepted `http` and `python` | `dcf/config/models.py` — new `PubSubSource` model with `subscription: str`; added to `Source` union; Pipeline validator enforces `strategy: append` for streaming | |
...
| F-030 | `deployment:` block in pipeline YAML silently ignored by `dcf validate` — invalid cron expressions passed without error | `config/models.py` — added `Deploy` model with cron validator; `Pipeline.deploy` optional field; `cli.py` validate now shows clean error on `ValidationError`; also fixed `from_dict` dict-mutation bug | |
...
| F-029 | `new-pipeline` skill had no mention of `catalog: gcp`, `dcf gcp setup`, or `dcf deploy` | `new-pipeline.md` — added Step 10 covering GCP prerequisites, required APIs, `deployment:` block syntax, and `dcf deploy`/`dcf undeploy` commands | |
