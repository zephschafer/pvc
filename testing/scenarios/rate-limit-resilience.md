...
Test pvc's behavior when it hits API rate limits. Fivetran handles rate limits
transparently (automatic backoff, retry, resume). pvc currently has no retry or
backoff logic — a 429 response is treated the same as any other HTTP error.

This scenario answers three questions:
1. What exactly happens when pvc hits a 429? (crash? log and continue? partial write?)
...
- Alternatively: use a token with a very short per-minute rate (pvc has a `rate_limit`
...
2. Run `pvc run github_commits_ratelimit`
3. Observe: when the 429 hits (after ~60 requests), what does pvc do?
...
pvc's YAML schema has a `rate_limit` config field. Test what it actually does:

1. Read `pvc/config/models.py` — what fields does `RateLimit` have?
2. Read `pvc/engine/fetcher.py` — how is `rate_limit` applied?
...
Note: pvc's `incremental` strategy upserts on primary key — it doesn't have a
...
- **Alternately:** pvc may have a `rate_limit` config with `per_minutes` — test
...
- **Expected Minor (UX):** The 429 error message is likely the same raw HTTP error
  as the 401 (F-009) — no pvc-specific guidance like "you've exceeded the API rate
...
- **Enhancement:** pvc could add automatic retry with exponential backoff for 429/503
...
- Check `pvc/config/models.py` for `RateLimit` model before designing the test —
