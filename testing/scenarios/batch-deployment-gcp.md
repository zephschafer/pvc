...
Test the new Terraform-based GCP batch deployment lifecycle: `ddt deploy github_repos` with
...
1. Does `ddt deploy github_repos` (GCP) build the pipeline container via Terraform
   (`gcloud builds submit`) and create a Cloud Run job named `ddt-job-github-repos`?
...
3. Does `ddt deploy` (GCP) also provision a Cloud Run Airflow service + Cloud SQL on first
...
5. Does `ddt undeploy github_repos` remove the Cloud Run job, DAG file, and — when it is
   the last pipeline — the Airflow Cloud Run service and Cloud SQL instance?

## Target Component

This scenario tests ddt's GCP provisioning layer:
- `ddt/infra/modules/gcp/batch_pipeline/` — rewritten to use Terraform + `gcloud builds submit`
- `ddt/infra/modules/gcp/airflow/` — new module: Cloud Run Airflow + Cloud SQL + GCS FUSE
- `ddt/infra/modules/templates/batch_pipeline.Dockerfile.tftpl` — shared template (`java_enabled=false` for GCP)
- `ddt/infra/modules/templates/airflow.Dockerfile.tftpl` — Airflow image with google provider
- `ddt/gcp/batch_deploy.py` — rewritten: no Composer calls; uses `_write_dag_gcs()` and `_tf_apply_airflow_gcp()`
...
9. Manually trigger the Cloud Run job to verify it executes correctly:
   ```bash
   gcloud run jobs execute ddt-job-github-repos --region <region> --wait
   ```
...
11. Test undeploy:
    ...
    gcloud run jobs list --region <region>           # ddt-job-github-repos gone
    gsutil ls gs://<bucket>/airflow/dags/            # github_repos.py gone
    ls ~/.ddt/terraform/pipelines/github_repos/gcp/  # No such file or directory
    gsutil ls gs://<bucket>/github_repos/            # warehouse data still present
    ```

Phase 1 success: `ddt deploy github_repos` (GCP) builds the image via `gcloud builds submit`,
creates `ddt-job-github-repos` Cloud Run job, writes `airflow/dags/github_repos.py` to GCS,
and exits 0 without any Composer references. `ddt undeploy` removes the job and DAG file
...
4. Confirm the `ddt-job-github-repos` Cloud Run job is updated to use the new image — not
...
2. Confirm Cloud SQL instance was created:
   ...
   # Expected: ddt-airflow-db  RUNNABLE  POSTGRES_15
   ```
3. Confirm Cloud Run Airflow service was created:
   ```bash
   gcloud run services list --region <region> --project <project_id>
   # Expected: ddt-airflow  running  min-instances=1
   ```
4. Get the Airflow Cloud Run service URL from the deploy output or:
   ```bash
   gcloud run services describe ddt-airflow --region <region> --format="value(status.url)"
   ```
...
11. Run `ddt deploy github_repos` a second time — Airflow stack should NOT be reprovisioned:
    ```bash
...
`ddt-job-github-repos` and writes data to GCS; second deploy leaves Airflow stack unchanged.

### Phase 4 — GCP Undeploy and Airflow Teardown

1. Run `ddt undeploy github_repos`:
   ```bash
   DDT_PROJECT_DIR=$CLONE uv --directory /path/to/ddt run ddt undeploy github_repos
   ```
2. Confirm the Cloud Run pipeline job is removed:
   ...
   # Expected: ddt-job-github-repos absent
   ```
...
4. Since `github_repos` was the only deployed pipeline, confirm the Airflow Cloud Run service
   ...
   # Expected: ddt-airflow absent
   gcloud sql instances list --project <project_id>
   # Expected: ddt-airflow-db absent (or DELETING)
   ```
5. Confirm Terraform state directories are removed:
   ```bash
   ls ~/.ddt/terraform/pipelines/github_repos/gcp/
   # Expected: No such file or directory
   ls ~/.ddt/terraform/airflow/gcp/
   # Expected: No such file or directory
   ```
...
7. To test the "partial undeploy" case (Airflow stays up when other pipelines remain):
   ...
   - `ddt deploy github_repos_2`
   - `ddt undeploy github_repos` (not the second pipeline)
   - Confirm Airflow Cloud Run and Cloud SQL remain running (second pipeline's DAG still in GCS)
   - `ddt undeploy github_repos_2`
...
- [ ] Phase 1: `ddt deploy github_repos` (GCP) exits 0; no Composer references in output
- [ ] Phase 1: `gcloud run jobs list` shows `ddt-job-github-repos` (NOT `pvc-job-github-repos`)
...
- [ ] Phase 1: Manual `gcloud run jobs execute ddt-job-github-repos` runs to completion
- [ ] Phase 1: Parquet files written to `gs://<bucket>/github_repos/github_repos/data/` after manual job execution
- [ ] Phase 1: `ddt undeploy github_repos` removes Cloud Run job and GCS DAG file; warehouse data untouched
- [ ] Phase 2: Second `ddt deploy github_repos` with unchanged files does NOT trigger `gcloud builds submit`
...
- [ ] Phase 3: Cloud SQL instance `ddt-airflow-db` created on first GCP deploy
- [ ] Phase 3: Cloud Run Airflow service `ddt-airflow` created with min-instances=1, gen2 execution environment
- [ ] Phase 3: Airflow UI accessible at Cloud Run HTTPS URL
- [ ] Phase 3: `github_repos` DAG appears in Airflow UI within ~30s of deploy
- [ ] Phase 3: Manually triggering DAG executes `ddt-job-github-repos`; data written to GCS warehouse
- [ ] Phase 3: Second `ddt deploy` does not reprovision Cloud SQL or Cloud Run Airflow service
- [ ] Phase 4: `ddt undeploy github_repos` (last pipeline): Cloud Run Airflow + Cloud SQL destroyed
- [ ] Phase 4: Airflow state directory removed (`~/.ddt/terraform/airflow/gcp/` gone)
...
- **`pvc-job-` → `ddt-job-` rename:** If a previous Terraform state at
  `~/.ddt/terraform/pipelines/github_repos/gcp/` references a resource named `pvc-job-...`,
  Terraform will plan to destroy the old job and create a new `ddt-job-...` one. This is
...
- **Cloud SQL Auth Proxy vs. direct VPC:** The `ddt-airflow` Cloud Run service connects to
...
- **Blocking (expected):** `ddt/infra/modules/gcp/batch_pipeline/main.tf` currently has a
  `google_storage_bucket_object.dag` resource (old Composer-based design). The new design
  removes this resource. Until the module is rewritten, `ddt deploy` (GCP) will still try to
...
- **Blocking (expected):** `gcp/batch_deploy.py` currently calls
  ...
  removed, `ddt deploy` (GCP) will fail trying to find/create a Composer environment. Document
  as Blocking finding.
...
- **Major (expected):** Cloud Run job may still be named `pvc-job-github-repos` if the
  ...
  `"ddt-job-${var.pipeline_name}"`. This will fail Phase 1 success criterion. Document as
...
- Record the exact Cloud Run job name returned by `gcloud run jobs list` in your findings.
  If it shows `pvc-job-...` rather than `ddt-job-...`, that is a pre-identified Major finding.
