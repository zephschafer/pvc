...
| F-059 | Minor | UX | `ddt undeploy` (local) does not tear down the Airflow stack when no DAGs remain — user must manually run `docker compose down` after the last undeploy. GCP path correctly destroys Airflow when no DAG files exist in GCS; local path has no equivalent check. | batch-deployment-local |
...
| F-039 | `ddt deploy` could not load or route streaming pipelines | `ddt/cli.py` — `deploy` and `undeploy` commands route by `pipeline.deployment.type`; `deploy-status` and confirm messages are streaming-aware | |
...
| F-030 | `deployment:` block in pipeline YAML silently ignored by `ddt validate` — invalid cron expressions passed without error | `config/models.py` — added `Deploy` model with cron validator; `Pipeline.deploy` optional field; `cli.py` validate now shows clean error on `ValidationError`; also fixed `from_dict` dict-mutation bug | |
...
| F-029 | `new-pipeline` skill had no mention of `catalog: gcp`, `ddt gcp setup`, or `ddt deploy` | `new-pipeline.md` — added Step 10 covering GCP prerequisites, required APIs, `deployment:` block syntax, and `ddt deploy`/`ddt undeploy` commands | |
