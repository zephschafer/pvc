...
- [x] Phase 1: `dcf validate` on a pipeline without `deploy:` is unaffected (no regression)
  - `dcf validate craigslist_apts` → OK
- [x] Phase 2: `dcf deploy github_repos` completes without error
  - New `dcf-composer` environment provisioned (27 min); Cloud Build 2m32s; total ~30 min
- [x] Phase 2: Cloud Composer DAG named `github_repos` is visible after deploy
  - `gs://us-central1-dcf-composer-59d3b6ad-bucket/dags/github_repos.py` confirmed
- [x] Phase 2: Cloud Run job for the pipeline exists after deploy
  - `dcf-job-github-repos` in `us-central1` (Ready) confirmed
...
- [x] Phase 2: `dcf deploy` on a pipeline with no `deployment:` block exits with a clear error
  - `dcf deploy craigslist_apts` → "has no 'deployment:' block in its pipeline YAML"
- [ ] Phase 2: `dcf deploy` without `catalog: gcp` in `project.yml` exits with a clear error
  - **STALE CRITERION**: `dcf deploy` with `catalog: local` now routes to local Docker
...
- [x] Phase 4: Second `dcf deploy` produces exactly one DAG (idempotent)
  - Exactly one `github_repos.py` in Composer bucket; one `dcf-job-github-repos` Cloud Run job
...
- [x] Phase 4: `dcf undeploy github_repos` removes the DAG and Cloud Run job
  - DAG file removed from Composer bucket; `dcf-job-github-repos` deleted from Cloud Run
- [x] Phase 4: Terraform state directory is removed after `dcf undeploy`
  - `~/.dcf/terraform/pipelines/github_repos/` — confirmed absent after undeploy
...
- `dcf-composer` environment auto-provisioned (first run in this GCP project since prior env was deleted)
...
2. **`dcf deploy` with `catalog: local` routed to local Docker instead of erroring.**
...
3. **No feature file for local Docker deployment.**
   The local deploy feature (commit `08faf16`) changed the CLI surface of `dcf deploy` but has
   no `features/local-deployment.md`. The FEATURES.md registry is now incomplete.
   [→ Finding F-048: Minor / UX]

4. **Prior `dcf-composer` environment was gone.**
   The prior run's `dcf undeploy` only removes the DAG (by design), but between runs the
...
| ID | Severity | Summary |
|----|----------|---------|
| F-047 | Minor | `features/batch-deployment.md` line 55 and scenario criterion "dcf deploy without catalog: gcp exits with clear error" are stale — behavior changed when local Docker deployment was added |
