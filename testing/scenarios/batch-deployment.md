...
**This scenario tests new feature code.** The `deployment:` block, `dcf deploy`, and
`dcf undeploy` commands do not exist yet — the first run will surface implementation
...
### Phase 2 — `dcf deploy` provisions GCP infrastructure via Terraform
...
2. Run `dcf deploy github_repos` — record the full output.
...
6. Query the warehouse:
   ...
   DCF_PROJECT_DIR=$CLONE uv --directory /path/to/dcf run python -c \
     "from dcf.warehouse_reader import query; print(query('SELECT COUNT(*), MAX(name) FROM github_repos.github_repos'))"
   ```
...
- [ ] Phase 4: Second `dcf deploy` produces exactly one DAG (idempotent)
- [ ] Phase 4: `dcf undeploy github_repos` removes the DAG and Cloud Run job
...
- **API enablement:** Cloud Composer, Cloud Run, and (if using container images)
  Artifact Registry APIs must be enabled before `dcf deploy` runs. If they aren't,
...
- **Terraform provider download time:** First `dcf deploy` on a new machine downloads
  the Google provider (~30 MB). Subsequent runs reuse the cache at
  `~/.dcf/terraform/.plugin-cache`. Slow first-deploy is expected, not a bug.
...
- If Cloud Composer is already provisioned from a previous run (check with
  `gcloud composer environments list`), note the environment name — `dcf deploy`
...
- If Phase 3 reveals a Cloud Run job execution failure, always check Cloud Run logs
  first (`gcloud logging read`) before diagnosing at the dcf layer.
